...
static int __init my_driver_init(void)
{
    ...
    {
		pr_warning("MYDRIVE: (sysfs) %s\n", sysfs_registration_error_messages[status]);
		return status;
	}
	pr_info("MYDRIVE: (sysfs) registered bus %s\n", my_bus_type.name);
	pr_info("MYDRIVE: (sysfs) registered driver %s on bus %s\n", mydriver.driver.name, my_bus_type.name);
	pr_info("MYDRIVE: (sysfs) added attributes command (available commands), input_command (user command input) on driver %s\n", mydriver.driver.name);
	...
	status = create_block_device(&device, DEFAULT_DEVICE_NAME, DEFAULT_DEVICE_CAPACITY);
	if (status < 0)
    {
		pr_warning("MYDRIVE: (defaultdevice) default device init failed\n");
		return status;
	}
	...
	if (status)
    {
		pr_warning("MYDRIVE: (defaultdevice) device registration on bus failed\n");
		return -status;
	}
