...
{
	struct user_device_list* lst = kmalloc(sizeof(struct user_device_list), GFP_KERNEL);
	lst -> device = dev;
	lst -> next = NULL;
	return lst;
}
...
{
	struct user_device_list* new_head = node_create(dev);
...
int block_dev_ioctl(struct block_device *bdev, fmode_t mode, unsigned cmd, unsigned long arg)
{
	return -ENOTTY;
}
...
static int do_transfer(struct request *rq, unsigned int *nr_bytes)
{
	...
	//pr_info("MYDRIVE: (request) request start from sector %lld  pos = %lld  dev_size = %lld\n", blk_rq_pos(rq), pos, dev_size);
...
static blk_status_t queue_rq(struct blk_mq_hw_ctx *hctx, const struct blk_mq_queue_data* bd)
{
	...
	if (do_transfer(bd -> rq, &nr_bytes) != 0 || blk_update_request(bd -> rq, status, nr_bytes))
    {
...
struct block_dev* allocate_device(struct block_dev **dev_pointer)
{
    *dev_pointer = kmalloc(sizeof(struct block_dev), GFP_KERNEL);
    if (!dev_pointer) return NULL;
    return *dev_pointer;
}

static int create_block_device(struct block_dev **dev_pointer, char *dev_name, sector_t capacity)
{
    int dev_major;
    struct block_dev *dev = allocate_device(dev_pointer);
    if (!dev)
    {
        pr_warning("MYDRIVE: (device) Unable to allocate device\n");
        return -ENOMEM;
    }
    ...
    {
        kfree(dev);
        pr_warning("MYDRIVE: (device) Unable to get major number\n");
        return -EBUSY;
    }
	...
	if (!dev -> data)
    {
        ...
        kfree(dev);
		pr_warning("MYDRIVE: (device) failed to allocate device IO buffer\n");
		return -ENOMEM;
	}

	pr_info("MYDRIVE: (device) initializing queue\n");
	dev -> queue = blk_mq_init_sq_queue(&dev -> tag_set, &mq_ops, QUEUE_DEPTH, BLK_MQ_F_SHOULD_MERGE);
	if (!dev -> queue)
    {
        unregister_blkdev(dev_major, dev_name);
        kfree(dev);
        kfree(dev -> data);
		pr_warning("MYDRIVE: (device) failed to allocate device queue\n");
		return -ENOMEM;
	}
